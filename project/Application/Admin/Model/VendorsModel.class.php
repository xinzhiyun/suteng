...
{   
    ...
    protected $_validate = array(
        array('name','/^[\w\x{4e00}-\x{9fa5}]{1,32}$/u','姓名称格式不对！',1,'regex'), // 真实用户名，不为空时验证，数字字母下划线中文 1-32位   
        ...
        // array('telephone','/^(((0\d{2,3})|(85[2|3]))\-)?([2-9]\d{6,7})(\-\d{1,4})?$/','固定电话格式不对！',2,'regex'), // 座机号码，不为空时验证，正确的座机号码支持 区号-电话号码-分机号码
        array('address','/^[\w\-\x{4e00}-\x{9fa5}]{6,255}$/u','公司地址格式不对！',1,'regex'), // 公司地址，不为空时验证，数字字母下划线横线中文 6-255位   
        array('user','','用户名已经存在！',0,'unique',1), // 在新增的时候验证user字段是否唯一
        array('user','/^[a-zA-Z][\w]{1,29}$/','用户名格式不对！',1,'regex'), // 字母开头，数字字母下划线，总长度 2-30位
